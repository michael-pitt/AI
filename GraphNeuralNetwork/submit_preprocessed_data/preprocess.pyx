...
cdef inline bint far_pairs(float r1, float r2, float z1, float z2, float phi1, float phi2, float x1, float x2, float y1, float y2):
    ...
    cdef float z0_max = 100 # previously used 150
    cdef float dphi = 0
...
cpdef _propogate_labels(long n_hits, ar[INT_64_t, ndim=1] outgouing, ar[INT_64_t, ndim=1] incomming, float[:] weight, ar[INT_64_t, ndim=1] labels, ar[INT_32_t, ndim=1] hid, float threshold):
    ...
    cdef INT_32_t[:] label_counts = np.zeros((n_hits), INT_32)
    labels[incomming[0]] = labels[outgouing[0]]
    label_counts[incomming[0]] = 1
    label_weight[incomming[0]] = weight[0]
    for i in range(1, size):
        #if weight[i]>THRESHOLD: weight[i] = 1
        if (weight[i] < THRESHOLD): continue
        label_counts[incomming[i]] += 1
        if( (incomming[i] == incomming[i-1]) and (weight[i] < label_weight[incomming[i]])
                    and (label_counts[outgouing[i]] < label_counts[incomming[i-1]])):
            #propogate the label back to incoming node:
            if( (labels[outgouing[i]] < labels[incomming[i]])): labels[outgouing[i]] = labels[incomming[i]]
            continue
...
def preprocess(hits, phi_bins, eta_bins, feature_names = ['x', 'y', 'z', 'phi', 'eta', 'r']):
    ...
    for i_eta in range(len(eta_bins)-1):  
      for i_phi in range(len(phi_bins)-1):
              
        
        _hits = (hits.loc[(hits['eta']>eta_bins[i_eta]) & (hits['eta']<=eta_bins[i_eta+1]) & (hits['phi']>phi_bins[i_phi]) & (hits['phi']<=phi_bins[i_phi+1])])
    ...
    #feature_names = ['x', 'y', 'z', 'phi', 'eta', 'layer']
